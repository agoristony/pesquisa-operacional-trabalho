...
class Simplex:
    ...
    def __init__(self, string_objective_function):
        ...
        self.objective = Objective.MAX.value # objetivo da função
    ...
    def calculate(self, table: list) -> None:
        ...
        # identifica a coluna pivô pelo menor ou maior valor
...
def graphical_solution(problem: Simplex):
    s1 = Symbol("x1")
    s2 = Symbol("x2")

    constraints = [constraint[1:problem.inserted+1] + [problem.column_b[i]] for i, constraint in enumerate(problem.table)]
    print(constraints)
