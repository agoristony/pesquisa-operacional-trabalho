...
class Simplex:
    ...
    def _build_objective_function(self, string_objective_function: str, maximize: bool = True) -> list:
    ...
    def add_restriction(self, expression: str):
        ...
        if delimiter == "<=":
            ...
            print(f'Variável artificial incluida: {self.artificial_vars} - {row}')
            constraint = self.insert_excess_var(row, default_format)
            
            print(f'Variável de excesso incluida: {self.excess_vars} - {constraint}')
    ...
    def calculate(self, table: list) -> None:
        ...
        self.entering_vars.append(f"{self.get_all_vars()[self.pivot_column_index - 1]}")
        ...
        self.basic_vars[first_exit_line - 1] = f"{self.get_all_vars()[self.pivot_column_index - 1]}"
        stack = table.copy()
    ...
    def show_table(self):
        ...
        print(tabulate(table_copy, tablefmt="fancy_grid", headers=["Base", "Z"] + [*self.get_all_vars()] + ["b"]))
    ...
    def two_phase(self):
        ...
        for i in range(num_variaveis):
            if f'x{i+1}' in self.artificial_vars:
                new_objective[i] = -1
            elif f'x{i+1}' in self.expression_util.get_variables(self.string_objective_function):
                new_objective[i] = sum(self.table[j][i+1] for j in range(1, len(self.table)))
            else:
                new_objective[i] = 0
        self.objective_function =  [1] + new_objective
        self.table[0] = self.objective_function + [sum(self.column_b)]
        self.show_table()
        self.calculate(self.table)
        ...
        self.show_table()
        self.objective = Objective.MAX.value
        self.calculate(self.table)
        return self.solve()
